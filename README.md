...
### 3.7 可选扩展：势能塑形（默认关闭，不参与上面的连续性论证）

上面的连续性论证依赖"奖励逐位不变"。当课程走到 **±180° 甩摆**时，纯 RL 的四次尝试都学不出抽能动作——
原因是物理的而非调参的：抽能律是**相位相干**的（`a = k(E−E*)θ̇cosθ`），而 i.i.d. 高斯探索满足
`E[a·θ̇·cosθ] = 0`，它只会"摇"杆子，不会"抽"能量。

于是加了一个**默认关闭**的奖励塑形开关 `--shaping energy`：

```
Φ(s) = -c·(E_pend(s) − E*)²        E* = 2mgl = 1.962 J
F(s,s') = γ·Φ(s') − Φ(s)            γ = 0.99（与 PPO 同）
```

它是 Ng/Harada/Russell (1999) 意义下的**势能塑形**：求和后只剩
`γΦ(s_T) − Φ(s_0) − (1−γ)ΣΦ(s_t)`，最后一项只依赖状态、对每个动作取同一个值，
因此 `argmax_a Q(s,a)` 不变——塑形**不能**让"倒下再翻上来"成为好策略，也无法被刷分。
`E_pend` 用 `env.pendulum_energy()`（车体坐标系里杆子的能量），**不是** `env.energy()`
（后者含小车动能，小车高速滑行时会读出 `25×E*` 而杆子几乎没动）。直立附近 `gap≈0` 且梯度为 0，
所以塑形在平衡区**恒等于零**。

**注意**：开启塑形后 `mean return` 与历史 run **不再可比**（塑形计入回报）。所有门槛与验收都用成功率，
与奖励无关，所以判定链不受影响。塑形版应作为**独立的一条实验线**记录，不要和 P1–P5 的回报数字混在一张表里。

实测（详见 [`RESULTS.md`](RESULTS.md)）：

| 判据 | 结果 |
|---|---|
| 平衡区是否被破坏 | **7 个分布 × 200 回合，逐回合完全相同**（同一检查点开/关塑形，不一致对数 0，p = 1.0） |
| 每步是否真的等于势差 | 131 步最大残差 7.1e-15；同状态 5 个动作的公共偏移散度 3.3e-16 J |
| 批量/标量是否一致 | 最大奖励差 7.1e-15（系数 0 与 1.0 都测） |
| 甩摆是否学会 | ±180° 起始：`(90°,135°]` **0% → 31.7%**、`(135°,180°]` **5% → 95%**（配对 McNemar，每带 60 回合） |
...
### 10.1 能力阶梯：成功率随初始角度的分布

单看"±90° 成功率"会藏住课程最需要的那个信息：**悬崖在哪里**。
`scripts/band_report.py` 在初始角度的窄带上分别评估，并给出 Wilson 区间；比较两个策略时用
**配对**精确 McNemar（同种子、同回合）——两个未配对、各 200 回合的成功率可以因种子噪声差 3–4 个点。

每带 60 回合，`--terminate-angle 0`，配对种子 900 起：

| 初始角度带 | P5 基底（纯平衡） | S1 塑形（hanging 起始） | S2 全角度（±180°） |
|---|---|---|---|
| (0°, 15°] | 100.0% | 100.0% | **100.0%** |
| (15°, 30°] | 100.0% | 100.0% | **100.0%** |
| (30°, 45°] | 95.0% | 96.7% | **100.0%** |
| (45°, 60°] | 53.3% | 45.0% | **73.3%**（p = 0.0005） |
| (60°, 90°] | 0.0% | 0.0% | **0.0%** |
| (90°, 135°] | 0.0% | 0.0% | **31.7%**（p < 0.0001） |
| (135°, 180°] | 5.0% | 36.7% | **95.0%**（p < 0.0001） |

**60–90° 这道墙的性质已查清：主要是导轨长度，不是策略能力。** 同一分布、同种子下把导轨从
±2.4 m 放宽到 ±6.0 m：

| 带 | ±2.4 m | 撞轨 | ±6.0 m | 撞轨 |
|---|---|---|---|---|
| (45°, 60°] | 70.0% | 30% | **100.0%** | 0% |
| (60°, 75°] | **3.3%** | 96.7% | **81.7%** | 18.3% |
| (75°, 90°] | 0.0% | 100% | **1.7%** | 98.3% |
| (90°, 135°] | 40.0% | 60% | **83.3%** | 16.7% |

读法有两条：**(60°,75°] 是空间问题**（±2.4 m 下 96.7% 的失败是撞轨，且 46% 的回合已经够到直立、
只差没保住）；**(75°,90°] 是真正的策略缺口**（给 6 m 导轨仍只有 1.7%，小车跑满 6.21 m 才出界）。
注意加长导轨会改变观测归一化（`_scale(s.x, x_limit)`），所以那一栏是**诊断用的分布外测量**，
不能当作策略的真实成绩。

完整的口径、配对检验、能量诊断与训练稳定性记录见 [`RESULTS.md`](RESULTS.md)。
